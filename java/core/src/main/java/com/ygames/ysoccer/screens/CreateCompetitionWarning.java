...
/** Warns that creating another competition will replace the currently loaded competition. */
class CreateCompetitionWarning extends GLScreen {
    ...
    CreateCompetitionWarning(GLGame game, Competition.Category createCategory) {
        ...
        String[] lines = CompetitionWarningText.split(msg);

        w = new Label();
        w.setText(lines[0], Font.Align.CENTER, Assets.font14);
        w.setPosition(game.gui.WIDTH / 2, 340);
        widgets.add(w);

        w = new Label();
        w.setText(lines[1], Font.Align.CENTER, Assets.font14);
